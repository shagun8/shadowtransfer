"""
Run inference on test images and save predictions for DINOv3
...
# Add parent directory (where script is located)
...
# Import dinov3 model (it will find dinov3/dinov3/models from the script_dir)
...
def load_model(checkpoint_path: str, device: torch.device, 
    ...
    # Try strict loading first, fallback for DINOv3
    try:
        model.load_state_dict(checkpoint['model_state_dict'], strict=True)
    except RuntimeError as e:
        print(f"⚠ Loading with strict=False for DINOv3 model (expected)")
...
def run_inference(model: nn.Module, dataloader: DataLoader, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Running inference")):
            ...
            # Get predictions - handle DINOv3 output format
...
def main():
    ...
    parser.add_argument('--dinov3_model_name', type=str, default=None,
                       help='DINOv3 model variant (overrides config)')
    parser.add_argument('--dinov3_weights_path', type=str, default=None,
                       help='Path to DINOv3 weights (overrides config)')
    parser.add_argument('--dinov3_pretrained', type=bool, default=None,
                       help='Use pretrained weights (overrides config)')
    parser.add_argument('--dinov3_frozen_stages', type=int, default=None,
                       help='Number of frozen stages (overrides config)')
    ...
    # Override DINOv3-specific parameters if specified
    if args.dinov3_model_name is not None:
        config['inference']['dinov3_model_name'] = args.dinov3_model_name
    if args.dinov3_weights_path is not None:
        config['inference']['dinov3_weights_path'] = args.dinov3_weights_path
    if args.dinov3_pretrained is not None:
        config['inference']['dinov3_pretrained'] = args.dinov3_pretrained
    if args.dinov3_frozen_stages is not None:
        config['inference']['dinov3_frozen_stages'] = args.dinov3_frozen_stages
    ...
    for eval_name, checkpoint_info in checkpoints_to_run.items():
        ...
        # Load model with DINOv3-specific parameters
        model = load_model(
            checkpoint_path, 
            device,
            model_name=config['inference']['dinov3_model_name'],
            weights_path=config['inference']['dinov3_weights_path'],
            pretrained=config['inference']['dinov3_pretrained'],
            frozen_stages=config['inference']['dinov3_frozen_stages']
        )
