"""
Post-hoc class-conditional temperature scaling on a trained DINOv3+SIB
checkpoint. Reads <ckpt_dir>/checkpoint_best.pth, writes
...
from train_dinov3_sib import (
    _compute_strict_metrics, _compute_tolerant_metrics, _average_metrics,
    collect_logits_and_labels_dinov3, fit_class_conditional_temperature,
...
def main():
    ...
    ckpt_path = os.path.join(args.checkpoint_dir, 'checkpoint_best.pth')
    if not os.path.exists(ckpt_path):
        raise FileNotFoundError(f'No checkpoint_best.pth in {args.checkpoint_dir}')
    ...
    loaders = get_dataloaders_sib(
        ...
        fda_L=getattr(train_args, 'fda_L', 0.01),
    )
    val_loader = loaders['val']
    test_loader = loaders['test']
    num_domains = loaders.get('num_domains', 2)
    ...
    model = DINOv3ShadowDetectorSIB(
        ...
        frozen_stages=train_args.frozen_stages,
        ...
        vib_beta_content=train_args.vib_beta_content,
        vib_beta_edge=train_args.vib_beta_edge,
        vib_beta_scale=train_args.vib_beta_scale,
        aug_sigma_style=train_args.aug_sigma_style,
        aug_sigma_shift=train_args.aug_sigma_shift,
    ...
    test_logits, test_labels, test_fnames = collect_logits_and_labels_dinov3(
