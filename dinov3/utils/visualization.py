...
plot_loss_curves  — overview panel + one subplot per loss component.
                    ...
                    small KL-like values are visible without compression.
...
def plot_loss_curves(train_losses, val_losses, save_path,
    ...
    Row 0 (full-width overview):
        All available losses plotted on a *shared* y-axis so their relative
    ...
    Row 1+ (individual panels, one per component):
        ...
        fine-grained decreases that would be invisible in the overview are
        clear.  Total loss is NOT shown here — only the components.

        Panels (when data is available):
          • Main (CE) Loss  — train_main + val (val is main CE in eval mode)
          • Auxiliary Loss  — train_aux only (no val equivalent)
    ...
    fig.suptitle('DINOv3 — Training Loss Curves', fontweight='bold',
                 fontsize=13, y=1.005)
...
def save_best_worst_visualizations(model, dataloader, device, output_dir,
    ...
    Args
    ----
    model        : trained MAMNet (will be set to eval mode)
