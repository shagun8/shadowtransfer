"""
Training script for OGLANet + ISW (Instance Selective Whitening).

Adds RobustNet-style ISW regularisation on top of OGLANet training.

Total loss = seg_total (loss1+...+loss6) + λ_ISW × ISW_loss

where ISW_loss is the per-image mean instance selective whitening loss
computed from GLAM encoder features at glam1, glam2, glam3.
...
Tolerant metrics are ALWAYS logged regardless of the decision flag.
"""
...
_dlog('train_oglanet_isw.py: SCRIPT STARTING')
...
_dlog('train_oglanet_isw.py: torch imported')
...
_dlog('train_oglanet_isw.py: all model/data imports done')

from utils.visualization_oglanet_isw import (
    plot_loss_curves_oglanet_isw,
    plot_metrics_curves,
    save_best_worst_visualizations,
)

_dlog('train_oglanet_isw.py: all imports complete')
...
# GLAMEncoder exposes glam1/glam2/glam3/glam4 (not layer1/layer2…)
# ──────────────────────────────────────────────────────────────────

class OGLANetEncoderFeatureHooks(EncoderFeatureHooks):
    """
    Forward hooks for OGLANet's GLAMEncoder.

    Maps logical feature names to the GLAM stage modules inside
    GLAMEncoder (self.glam1, self.glam2, etc.) rather than the plain
    ResNet layer names used in MAMNet.
    """
    _LAYER_MAP = {
        'feat1': 'glam1',
        'feat2': 'glam2',
...
def get_args():
    p = argparse.ArgumentParser(description='Train OGLANet + ISW')
    ...
    # Training (paper Section 3.2)
    p.add_argument('--epochs', type=int, default=100)
    p.add_argument('--lr', type=float, default=0.0005,
                   help='Learning rate (Adamax, as per OGLANet paper)')
    ...
    p.add_argument('--eval_boundary_tolerant', action='store_true',
                   help='Use tolerant mIOU for all decisions. '
                        'DetailedEvaluator ALWAYS runs; this flag only '
                        'controls which metric drives LR/checkpoint/ES.')
    ...
    p.add_argument('--isw_lambda', type=float, default=0.6,
                   help='Weight λ for ISW loss (default: 0.6 from paper)')
    p.add_argument('--isw_layers', type=str, default='feat1,feat2,feat3',
                   help='Comma-separated GLAM encoder layers for ISW '
...
class TrainerOGLANetISW:
    """Trainer for OGLANet + ISW."""
    ...
    def __init__(self, args):
        ...
        if args.mode == 'single':
            ...
            exp_name = f'oglanet{modifier_str}_{city}_{res}_1'
        elif args.mode == 'all':
            exp_name = f'oglanet{modifier_str}_all_{args.resolution}_1'
        elif args.mode == 'loco':
            from data.dataset import LOCO_FOLDS
            test_city = LOCO_FOLDS[args.fold_id]['test']
            exp_name = (f'oglanet{modifier_str}_loco_holdout_'
        ...
        # ── Model ─────────────────────────────────────────────────
        print('Initialising OGLANet model...')
        print('ASSUMPTION: ResNet-34 encoder (not ResNet-101) for fair comparison.')
        self.model = OGLANet(
        ...
        # ── ISW loss + OGLANet-specific encoder hooks ─────────────
        isw_layers = args.isw_layers.split(',')
        print(f'ISW layers: {isw_layers}  (map: feat→glam)  λ={args.isw_lambda}')
        ...
        # OGLANetEncoderFeatureHooks maps feat1→glam1, feat2→glam2, feat3→glam3
        self.encoder_hooks = OGLANetEncoderFeatureHooks(
            self.model, layer_names=isw_layers)

        # ── Losses & optimiser ────────────────────────────────────
        self.criterion = OGLANetLoss()

        # Adamax as per OGLANet paper Section 3.2
        ...
        # Decision metric: tolerant mIOU drives the scheduler
        ...
        if self.use_tolerant_decision:
            ...
            print('>> Decision metric: STRICT per-image mIOU')

        # DetailedEvaluators always on
        ...
        # ── Tracking variables ────────────────────────────────────
        self.start_epoch                = 0
        self.best_miou                  = 0.0    # pooled ref
        self.best_shadow_iou            = 0.0    # pooled ref
        self.best_f1                    = 0.0    # pooled ref
        self.best_decision_miou         = 0.0    # drives checkpoint/ES
        ...
        self.train_loss5       = []
        self.train_loss6       = []
        self.train_isw_losses  = []   # λ × isw (train)
        self.val_losses        = []   # val CE on P6
        self.val_isw_losses    = []   # λ × isw (val, monitoring)
    ...
    def train_epoch(self, epoch):
        """
        Train for one epoch.
        Returns (epoch_loss, epoch_isw, seg_component_dict, pooled_metrics)
        """
        self.model.train()

        epoch_total   = 0.0
        epoch_isw     = 0.0
        epoch_seg     = {'loss1': 0., 'loss2': 0., 'loss3': 0.,
                         'loss4': 0., 'loss5': 0., 'loss6': 0.}
        ...
        for batch_idx, batch in enumerate(self.dataloaders['train']):
            ...
            # Forward — hooks capture glam1/2/3 features automatically
            outputs = self.model(images)    # dict {'p1'...'p6'} in train mode

            # Segmentation loss (6 outputs → 6 losses + total)
            seg_losses = self.criterion(outputs, masks)

            # ISW loss from hooked GLAMEncoder features
            ...
            # Metrics — use filtered P6 predictions (consistent with val/test)
            ...
            epoch_isw   += isw_weighted.item()
            for key in epoch_seg:
                epoch_seg[key] += seg_losses[key].item()
        ...
        epoch_isw   /= num_batches
        for key in epoch_seg:
            epoch_seg[key] /= num_batches

        metrics    = train_metrics.compute()
        epoch_time = time.time() - t0

        print(f'\nTraining Results:')
        print(f'Time: {epoch_time:.2f}s | Total: {epoch_total:.4f} | '
              f'ISW: {epoch_isw:.4f}')
        print(f'  ' + '  '.join(f'{k}={v:.4f}' for k, v in epoch_seg.items()))
        ...
        # TensorBoard — losses
        self.writer.add_scalar('Train/TotalLoss', epoch_total, epoch)
        self.writer.add_scalar('Train/ISWLoss',   epoch_isw,   epoch)
        for key, val in epoch_seg.items():
        ...
        # Store histories
        self.train_losses.append(epoch_total)
        self.train_isw_losses.append(epoch_isw)
        self.train_loss1.append(epoch_seg['loss1'])
        self.train_loss2.append(epoch_seg['loss2'])
        self.train_loss3.append(epoch_seg['loss3'])
        self.train_loss4.append(epoch_seg['loss4'])
        self.train_loss5.append(epoch_seg['loss5'])
        self.train_loss6.append(epoch_seg['loss6'])
        for key in self.train_metrics_history:
            self.train_metrics_history[key].append(metrics[key])

        return epoch_total, epoch_isw, epoch_seg, metrics
    ...
    def validate(self, epoch):
        """
        Validate the model.
        Returns (val_loss, val_isw_loss, pooled_metrics, detailed_results)
        """
        print('\nValidating...')
        self.model.eval()
        ...
        with torch.no_grad():
            for batch in self.dataloaders['val']:
                ...
                # In eval mode OGLANet returns P6 tensor directly
                ...
                # ISW monitoring — hooks still fire in eval mode
                isw_raw       = self.isw_loss_module(self.encoder_hooks.features)
                val_isw_loss += (self.args.isw_lambda * isw_raw).item()

                # Filtered predictions for metrics (consistent with train/test)
        ...
        print(f'Loss (CE on P6): {val_loss:.4f} | '
              f'ISW (monitor, λ×): {val_isw_loss:.4f}')
        print(f'OA: {metrics["OA"]:.2f}%  Precision: {metrics["Precision"]:.2f}%  '
              f'F1: {metrics["F1"]:.2f}%  BER: {metrics["BER"]:.2f}%  '
              f'mIOU(pooled): {metrics["mIOU"]:.2f}%  '
              f'Shadow_IOU: {metrics["Shadow_IOU"]:.2f}%')
    ...
    def train(self):
        print('\n' + '=' * 50)
        print('Starting training (OGLANet + ISW)...')
        metric_label = (f'Tolerant (±{self.args.boundary_tolerance}px) mIOU'
                        if self.use_tolerant_decision else 'Strict per-image mIOU')
        ...
        for epoch in range(self.start_epoch, self.args.epochs):
            ...
            # Train
            (train_total, train_isw,
             train_seg, train_metrics) = self.train_epoch(ep)

            # Validate
            (val_loss, val_isw,
             val_metrics, detailed) = self.validate(ep)
            ...
            # Track pooled bests (reference only)
...
def main():
    ...
    args    = get_args()
    _dlog(f'Args: mode={args.mode}, isw_mask_dir={args.isw_mask_dir}')
    _dlog('Creating TrainerOGLANetISW...')
    trainer = TrainerOGLANetISW(args)
    _dlog('Trainer created successfully.')
