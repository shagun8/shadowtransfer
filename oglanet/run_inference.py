"""
Run inference on test images and save predictions for OGLANet
...
def load_model(checkpoint_path: str, device: torch.device, img_size: int = 384) -> nn.Module:
...
def run_inference(model: nn.Module, dataloader: DataLoader, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Running inference")):
            ...
            # Get predictions - handle OGLANet output format
...
def main():
    ...
    for eval_name, checkpoint_info in checkpoints_to_run.items():
        ...
        # Load model
        model = load_model(checkpoint_path, device, config['inference']['img_size'])
