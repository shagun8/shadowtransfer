...
plot_loss_curves  — overview panel (all losses on shared y-axis) +
                    one subplot per loss component on its own y-axis scale.
...
def plot_loss_curves(train_losses, val_losses, save_path, component_losses=None):
    ...
    Row 1+ (individual panels, one per component + one for val):
        Each panel shows a *single* loss with its own y-axis so fine-grained
        decreases are clear.  Total loss is NOT shown here.

        Panels (when data is available):
          • Val Total Loss  — val only (its own scale)
          • loss1 … loss6   — train only (each at its own scale)
    ...
    component_losses : dict | None  e.g. {'loss1': [...], ..., 'loss6': [...]}
    ...
    # ---- Build individual panel specs (NO total loss panel) ----
    individual_panels = []

    # Val total loss — its own panel so its scale is visible
    if val_losses and len(val_losses) == len(epochs):
        individual_panels.append({
            'title': 'Val Total Loss',
            'ylabel': 'Loss',
            'series': [
                (val_losses, 'Val Total', _C_VAL_TOTAL, '--', 's'),
            ],
        })
