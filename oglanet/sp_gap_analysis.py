"""
sp_gap_analysis.py  —  Phase 1 SP-gap measurement for OGLANet C4-clean.

Same methodology as mamnet/sp_gap_analysis.py, adapted to OGLANet:
  - OGLANetSIB returns dict {'predictions': {'p1'..'p6'}, 'kl_losses': ...}
    We use 'p6' (full-resolution main output) as the logits for C4-clean.
  - Vanilla OGLANet AURC will be computed in the aggregator from saved .npy
...
Per-image AURC_shadow on ground-truth shadow pixels:
  ...
  Lower = model is more confidently correct on shadow.

Bootstrap unit: IMAGE (n=150 per cell). Pixel correlation within an image
is absorbed — each image contributes one observation. B=10,000 resamples.
Two-sided p = 2 * fraction of bootstrap means on opposite side of zero.

This script writes a per-cell JSON with C4-clean AURC, ECE, mIoU
distributions. The aggregator (aggregate_sp_gap.py) then loads these
plus the .npy-derived metrics for Vanilla + 6 adaptation methods, runs
the cross-method comparisons, and runs the population-level Wilcoxon.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Phase 1 SP-gap analysis for OGLANet C4-clean')

    p.add_argument('--c4clean_checkpoint', type=str, required=True,
                   help='Path to C4-clean OGLANetSIB best_model.pth')
...
def load_c4clean_oglanet_sib(checkpoint_path, device):
    """
    Load an OGLANetSIB from a C4-clean checkpoint.
    Checkpoints saved by train_oglanet_sib.py contain 'args' dict that
    fully specifies the SIB configuration; we rebuild architecture from it.
    """
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            f'C4-clean checkpoint not found: {checkpoint_path}')

    print(f'Loading C4-clean OGLANetSIB: {checkpoint_path}')
    ckpt = torch.load(checkpoint_path, map_location=device, weights_only=False)

    if 'args' not in ckpt:
        raise KeyError(
            "C4-clean checkpoint missing 'args' key. "
            "Expected a checkpoint saved by train_oglanet_sib.py.")
    ...
    model = OGLANetSIB(
        ...
        pretrained_encoder=False,   # weights overridden by load_state_dict
        use_sib=getattr(train_args, 'use_sib',
                        train_args.use_haar or train_args.use_vib),
        sib_channels=512,
        beta_content=train_args.beta_content,
        beta_edge=train_args.beta_edge,
        beta_noise=getattr(train_args, 'noise_scale', 0.05),
        ...
        sigma_style=getattr(train_args, 'sigma_style', 0.1),
        sigma_shift=getattr(train_args, 'sigma_shift', 0.05),
...
def run_c4clean_inference(model, loader, device):
    """
    Run C4-clean OGLANetSIB on the test loader.
    OGLANetSIB returns dict; main logits are at predictions['p6'].
    ...
    for batch in loader:
        ...
        # OGLANetSIB main logits = predictions['p6']
        logits = out['predictions']['p6'].float().cpu()
...
# =====================================================================
# Per-image metric functions (identical to MAMNet version)
# =====================================================================
...
def main():
    ...
    print(f'\n{"="*65}')
    print(f'OGLANet SP-Gap Phase 1 (C4-clean only)')
    print(f'  Architecture : OGLANet')
    print(f'  Holdout city : {test_city}')
    print(f'  Resolution   : {args.resolution}')
    print(f'  Bootstrap B  : {args.bootstrap_B}')
    print(f'  Min shadow px: {args.min_shadow_pixels}')
    print(f'  Coverage grid: {args.n_coverage} levels (0.10..1.00)')
    print(f'{"="*65}')
    ...
    loaders = get_dataloaders_sib(
        ...
        use_contrast=True,
        use_fda=False,    # FDA was training-only
        fda_L=0.005,
    )
    test_loader = loaders['test_loader']
    print(f'  Test images: {len(test_loader.dataset)}')

    model, c4_args = load_c4clean_oglanet_sib(args.c4clean_checkpoint, device)
    ...
    # Save per-image probability maps for the aggregator's bookkeeping
    # (so the aggregator can pair C4-clean with the saved .npy baselines
    # by filename without re-running inference).
    probs_save_dir = os.path.join(args.output_dir, f'c4clean_probs_oglanet_{test_city}_{args.resolution}')
    ...
    for i in range(n_images):
        ...
        # Save C4-clean probability map (filename matches saved .npy convention)
    ...
    # Save per-image arrays as CSV for the aggregator
    per_image_records = []
    for i in range(n_images):
    ...
    val_loader = loaders['val_loader']  # CONFIRM THIS KEY IN dataset_sib.py
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            city_ids      = batch['city_id'].to(device)
            out = model(images, intensity_map=intensity_map, city_ids=city_ids)
            c4_logits = out['predictions']['p6'].float().cpu()
    ...
    out_path = os.path.join(
        args.output_dir,
        f'sp_gap_oglanet_c4clean_{test_city}_{args.resolution}.json')
